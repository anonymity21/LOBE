...
def run_and_compare(mlir_file):
    ...
    # Step 2: run JIT
    jit_cmd = f'timeout 100 {config.jit_runner} {llvm_file} {config.jit_arg}  1> {result_file} 2> {err_file}'
    util.execmd(jit_cmd)
...
# def main():
#     args = parse_args()
#     all_mlirfiles = util.get_all_files_in_directory(f'{config.project_dir}/{seed_name}')
#     random.shuffle(all_mlirfiles)

#     start_time = time.time()
#     time_limit = 60 * 60 * 1 
#     count = 0
#     for mlir_file in tqdm(all_mlirfiles, desc="Processing MLIR files"):
#         current_time = time.time()
#         if current_time - start_time >= time_limit:
#             print(f"Reached 1-hour time limit. Processed {count} files.")
#             break
#         run_and_compare(mlir_file)
#         count += 1
#     elapsed_time = time.time() - start_time
#     print(f"Total executed files: {count}")
#     print(f"Total elapsed time: {elapsed_time:.2f} seconds")
        
    # with multiprocessing.Pool(processes=args.multiprocess) as pool:
    #     pool.map(run_and_compare, all_mlirfiles)
