...
class ExecutionResult:
    ...
    def process_multi_results(self, raw_text):
        ...
        for line in raw_text.splitlines():
            if line.startswith('Unranked Memref'):
                if current_block:
                    results.append('\n'.join(current_block))
                    current_block = []
        ...
        for block in results:
            if 'Memref base@' in block and 'data =' in block:     
                cleaned_blocks.append(block.split('data =')[1].strip())
        return cleaned_blocks

    

def process_multi_results(result):
    new_result = []
    for line in result.split('\n'):
        if not 'Unranked Memref' in line:
...
# python3 tosa_code/main_tosa.py --strategy=fullreset --multiprocess=1 --executiontime=0.5 --seedname=tosa_seed_v1
